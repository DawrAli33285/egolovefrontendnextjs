...
function parseCSV(text: string): QuizQuestion[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (cols.length < 15) { console.warn(`Row ${i}: only ${cols.length} cols`); continue; }
    try {
      results.push({
        id: i, langue, persona,
    ...
    } catch (err) { console.warn(`Row ${i} parse error:`, err); }
  }
  return results;
}
...
export default function AdminQuizPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ id: number; question_id: string; question: string } | null>(null);
  const [deleting, setDeleting]         = useState(false);

  // ── DB questions (loaded from API for the table)
  const [dbQuestions, setDbQuestions]   = useState<QuizQuestion[]>([]);
  const [dbLoading, setDbLoading]       = useState(false);
  const [viewMode, setViewMode]         = useState<'uploaded' | 'database'>('uploaded');

  // ── Admin session (for logout display)
  ...
  // ── Load DB questions when switching to database view
  const loadDbQuestions = useCallback(async () => {
    setDbLoading(true);
    try {
      const res = await fetch(`/api/admin/quiz/import?quizType=${activeType}&langue=FR&lang=fr`);
      const data = await res.json();
      setDbQuestions(data.questions || []);
    } catch {
      showToast('Failed to load questions from DB', 'err');
    } finally {
      setDbLoading(false);
    }
  ...
  useEffect(() => {
    if (tab === 'table' && viewMode === 'database') {
      loadDbQuestions();
    }
  }, [tab, viewMode, activeType, loadDbQuestions]);

  const displayQuestions = viewMode === 'database' ? dbQuestions : slots[activeType].questions;
  ...
  useEffect(() => {
    ...
    setFiltered(data);
  }, [displayQuestions, filterPillar, filterPersona, search]);
  ...
  const processFile = useCallback((file: File, type?: QuizType) => {
    ...
    if (!type) { setPendingFile(file); setShowTypeModal(true); return; }
    setImporting(type);
    const reader = new FileReader();
    reader.onload = (e) => {
      const text = e.target?.result as string;
      const parsed = parseCSV(text);
      if (parsed.length === 0) { showToast('No valid rows found', 'err'); setImporting(null); return; }
      setSlots(prev => ({ ...prev, [type]: { ...prev[type], questions: parsed, fileName: file.name } }));
      setImporting(null);
      setActiveType(type);
      setViewMode('uploaded');
      setTab('table');
      showToast(`✓ Parsed ${parsed.length} ${type} questions`);
    };
  ...
  const confirmTypeAndProcess = (type: QuizType) => {
    ...
    if (pendingFile) { processFile(pendingFile, type); setPendingFile(null); }
  };
  ...
  const handleSave = async (type: QuizType) => {
    ...
    try {
      ...
      setTab('stats');
      showToast(`✓ ${data.inserted} questions saved`);
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/quiz/${deleteTarget.id}`, { method: 'DELETE' });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.message || 'Delete failed');
      }
      // Remove from local state
      setDbQuestions(prev => prev.filter(q => q.id !== deleteTarget.id));
      setDeleteTarget(null);
      showToast(`✓ Deleted ${deleteTarget.question_id}`);
    } catch (err: any) {
  ...
  const pillarStats: PillarStats[] = Array.from(
    slots[activeType].questions.reduce((acc, q) => {
      if (!acc.has(q.pilier_id)) acc.set(q.pilier_id, { id: q.pilier_id, nom: q.pilier_nom, emoji: q.pilier_emoji, count: 0 });
      acc.get(q.pilier_id)!.count++;
      return acc;
      ...
      <div className={`flex-1 rounded-2xl border-2 p-6 flex flex-col gap-4 transition-all
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className={`text-lg ${isFree ? 'text-violet-400' : 'text-amber-400'}`}>{isFree ? '🆓' : '⭐'}</span>
            <span className={`text-sm font-black uppercase tracking-wider ${isFree ? 'text-violet-300' : 'text-amber-300'}`}>{type} Quiz</span>
            <span className="text-[10px] text-slate-600">{isFree ? '· 45 questions' : '· 198 questions'}</span>
          </div>
          {loaded && <span className="text-[10px] text-slate-500 font-mono">{slot.questions.length} loaded</span>}
        </div>
        <div
          onDragOver={(e) => { e.preventDefault(); setDragOver(type); }}
          onDragLeave={() => setDragOver(null)}
          onDrop={(e) => { e.preventDefault(); setDragOver(null); const f = e.dataTransfer.files[0]; if (f) processFile(f, type); }}
        ...
        >
          <input ref={ref} type="file" accept=".csv" className="hidden"
            onChange={e => { const f = e.target.files?.[0]; if (f) processFile(f, type); e.target.value = ''; }} />
          {isLoading ? (
            <div className="flex flex-col items-center gap-2">
              <div className={`w-8 h-8 border-2 rounded-full animate-spin ${isFree ? 'border-violet-500/30 border-t-violet-400' : 'border-amber-500/30 border-t-amber-400'}`} />
            ...
            <>
              ...
              <p className="text-slate-500 text-[10px]">or click to browse</p>
            </>
          )}
        </div>
        {loaded && (
          <div className="flex gap-2">
            <button onClick={() => { setActiveType(type); setViewMode('uploaded'); setTab('table'); }}
            ...
            <button onClick={() => handleSave(type)} disabled={isSaving || saving !== null}
              ...
              {isSaving
                ? <span className="flex items-center justify-center gap-1.5"><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</span>
                : '💾 Save to DB'}
            </button>
          </div>
        )}
        {slot.stats && (
          <div className="bg-emerald-950/30 border border-emerald-800/40 rounded-xl px-3 py-2 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-mono">{slot.stats.inserted} inserted · {slot.stats.skipped} skipped</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-slate-200 font-mono">
      ...
      {/* ══ TOAST ══ */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[999] px-5 py-3 rounded-xl text-sm font-bold shadow-2xl
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[998] flex items-center justify-center p-5 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0d1220] border border-red-900/50 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-2xl">🗑️</span>
              <h3 className="text-lg font-black text-red-300">Delete Question?</h3>
            </div>
            <div className="bg-slate-900/60 rounded-xl p-4 mb-5">
              <div className="text-[10px] text-slate-500 font-mono mb-1">{deleteTarget.question_id}</div>
              <p className="text-xs text-slate-300 leading-relaxed line-clamp-3">{deleteTarget.question}</p>
            </div>
            <p className="text-xs text-slate-500 mb-6">
              This will permanently remove the question and all its translations from the database. This cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteTarget(null)}
                disabled={deleting}
                className="flex-1 text-xs px-4 py-3 rounded-xl border border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200 transition disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 text-xs px-4 py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-black transition disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {deleting
      ...
      {showTypeModal && (
        <div className="fixed inset-0 z-[997] flex items-center justify-center p-5 bg-black/75 backdrop-blur-sm">
          <div className="bg-[#0d1220] border border-slate-700 rounded-2xl p-8 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-slate-500 text-xs mb-6">
              Select the type for{' '}
              <span className="text-slate-300 font-mono bg-slate-800 px-1.5 py-0.5 rounded">{pendingFile?.name}</span>
            </p>
            <div className="flex flex-col gap-3 mb-5">
              <button onClick={() => confirmTypeAndProcess('free')}
                className="flex items-center gap-4 p-4 rounded-xl border-2 border-violet-700 bg-violet-950/30 hover:border-violet-400 hover:bg-violet-950/50 text-left transition-all hover:scale-[1.01]">
                <span className="text-3xl">🆓</span>
                <div>
                  <div className="font-black text-sm uppercase tracking-wider text-violet-300">Free Quiz</div>
                  <div className="text-xs text-slate-500 mt-0.5">45 questions · 4-choice answers</div>
                </div>
              </button>
              <button onClick={() => confirmTypeAndProcess('premium')}
                className="flex items-center gap-4 p-4 rounded-xl border-2 border-amber-700 bg-amber-950/30 hover:border-amber-400 hover:bg-amber-950/50 text-left transition-all hover:scale-[1.01]">
                <span className="text-3xl">⭐</span>
                <div>
                  <div className="font-black text-sm uppercase tracking-wider text-amber-300">Premium Quiz</div>
                  <div className="text-xs text-slate-500 mt-0.5">198 questions · Likert scale</div>
                </div>
              </button>
            </div>
            <button onClick={() => { setShowTypeModal(false); setPendingFile(null); }}
      ...
      <header className="border-b border-slate-800 bg-[#0a0f1a]/90 backdrop-blur sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-xs font-black text-white">A</div>
          ...
          <div className="flex items-center gap-3">
            {(slots.free.questions.length > 0 || slots.premium.questions.length > 0) && (
              <div className="flex rounded-lg border border-slate-700 overflow-hidden text-[10px] font-bold">
                {(['free', 'premium'] as QuizType[]).map(t => (
                  <button key={t} onClick={() => setActiveType(t)} disabled={slots[t].questions.length === 0}
                    ...
                    {t === 'free' ? '🆓' : '⭐'} {t}
                    {slots[t].questions.length > 0 && <span className="ml-1 opacity-60">({slots[t].questions.length})</span>}
                  </button>
                ))}
              </div>
            )}
            {filtered.length > 0 && tab === 'table' && (
            ...
            <button onClick={handleLogout}
              className="text-xs px-3 py-1.5 rounded-lg border border-slate-700 text-slate-500 hover:border-red-800 hover:text-red-400 transition flex items-center gap-1.5">
              <span>⏏</span> Logout
            </button>
          </div>
        </div>
          ...
          {(['import', 'table', 'stats'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 text-xs font-bold rounded-t-lg capitalize transition border-b-2
                ${tab === t ? 'text-violet-300 border-violet-500 bg-violet-950/30' : 'text-slate-500 border-transparent hover:text-slate-300'}`}>
              {t === 'import' ? '📂 Import' : t === 'table' ? `📋 Questions${filtered.length ? ` (${filtered.length})` : ''}` : '📊 Stats'}
            </button>
          ))}
        </div>
      </div>
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-6">
        ...
        {tab === 'import' && (
          <div className="max-w-3xl mx-auto mt-8">
            ...
            <p className="text-slate-500 text-sm mb-8">Drop any CSV to be asked the type, or use the specific zones below.</p>

            {/* Generic drop zone */}
            <div
              onDragOver={(e) => { e.preventDefault(); setDragOver('generic'); }}
              onDragLeave={() => setDragOver(null)}
              onDrop={(e) => { e.preventDefault(); setDragOver(null); const f = e.dataTransfer.files[0]; if (f) processFile(f); }}
              ...
              className={`border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all mb-6
                ${dragOver === 'generic' ? 'border-slate-400 bg-slate-800/40 scale-[1.01]' : 'border-slate-700 bg-slate-900/30 hover:border-slate-500 hover:bg-slate-800/30'}`}
            >
              <input ref={genericFileRef} type="file" accept=".csv" className="hidden"
            ...
            <div className="mt-5 bg-indigo-950/40 border border-indigo-900/50 rounded-xl p-4 flex gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  French questions are translated to <strong className="text-slate-300">English</strong> and{' '}
                  <strong className="text-slate-300">Spanish</strong> on save. Large files (~198 Qs) take ~3 min.
                </p>
              </div>
            </div>
        ...
        {tab === 'table' && (
          <>
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              ...
              <div className="flex gap-2">
                {(['free', 'premium'] as QuizType[]).map(t => (
                  <button key={t} onClick={() => setActiveType(t)}
                    className={`text-xs px-4 py-2 rounded-lg font-bold capitalize transition border
                      ${activeType === t
              ...
              {/* View mode toggle */}
              <div className="flex rounded-lg border border-slate-700 overflow-hidden text-[10px] font-bold ml-2">
                <button onClick={() => setViewMode('uploaded')}
                  disabled={slots[activeType].questions.length === 0}
                  className={`px-3 py-1.5 transition disabled:opacity-30
                    ${viewMode === 'uploaded' ? 'bg-slate-700 text-slate-100' : 'text-slate-500 hover:text-slate-300'}`}>
                  📤 Uploaded
                </button>
                <button onClick={() => { setViewMode('database'); loadDbQuestions(); }}
                  className={`px-3 py-1.5 transition ${viewMode === 'database' ? 'bg-slate-700 text-slate-100' : 'text-slate-500 hover:text-slate-300'}`}>
                  🗄 Database
                </button>
              </div>

              {viewMode === 'database' && (
                <button onClick={loadDbQuestions} disabled={dbLoading}
                  className="text-[10px] px-3 py-1.5 rounded-lg border border-slate-700 text-slate-500 hover:text-slate-300 transition disabled:opacity-40">
            ...
            <div className="flex flex-wrap gap-3 mb-5 items-center">
              <input type="text" placeholder="🔍 Search questions, pillars, IDs…" value={search}
              ...
              <select value={filterPillar} onChange={e => setFilterPillar(e.target.value)}
                ...
                {uniquePillars.map(p => <option key={p.id} value={p.id}>{p.emoji} {p.nom}</option>)}
              </select>
              ...
              {(search || filterPillar || filterPersona) && (
                <button onClick={() => { setSearch(''); setFilterPillar(''); setFilterPersona(''); }}
            ...
            {dbLoading && viewMode === 'database' ? (
              <div className="flex items-center justify-center py-24 text-slate-600">
                <div className="w-8 h-8 border-2 border-slate-700 border-t-violet-500 rounded-full animate-spin mr-3" />
              ...
              <div className="text-center py-24 text-slate-600">
                ...
                <p className="font-bold">
                  {viewMode === 'database' ? 'No questions in database for this type' : 'No questions uploaded yet'}
                </p>
                {viewMode === 'uploaded' && (
                  <button onClick={() => setTab('import')} className="mt-4 text-xs px-4 py-2 rounded-lg bg-violet-600 text-white hover:bg-violet-500 transition">
                    → Import CSV
                  </button>
                )}
              </div>
                ...
                <table className="w-full min-w-[1500px] text-xs border-collapse">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-[#0d1220] border-b border-slate-800">
                      {['ID', 'Type', 'Pillar', 'Persona', 'Weight', 'Label', 'Question', 'A (1pt)', 'B (2pt)', 'C (3pt)', 'D (4pt)',
                        viewMode === 'database' ? 'Delete' : ''].filter(Boolean).map(h => (
                        <th key={h} className="text-left px-3 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500 whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((q, i) => {
                      ...
                      return (
                        <tr key={q.id} className={`border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors group ${i % 2 === 0 ? 'bg-transparent' : 'bg-slate-900/20'}`}>
                          <td className="px-3 py-2.5 font-mono font-bold text-slate-400 whitespace-nowrap">{q.question_id}</td>
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${activeType === 'free' ? 'bg-violet-950/40 text-violet-300 border-violet-800/50' : 'bg-amber-950/40 text-amber-300 border-amber-800/50'}`}>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold border ${pc.bg} ${pc.text} ${pc.border}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${pc.dot}`} />{q.pilier_emoji} {q.pilier_nom}
                            </span>
                          </td>
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${PERSONA_COLORS[q.persona] || PERSONA_COLORS.All}`}>{q.persona}</span>
                          </td>
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            <span className={`font-mono font-bold ${q.poids_question >= 2 ? 'text-amber-400' : 'text-slate-400'}`}>×{q.poids_question}</span>
                          </td>
                          ...
                          {/* Delete button — only in database view */}
                          {viewMode === 'database' && (
                            <td className="px-3 py-2.5 whitespace-nowrap">
                              <button
                                onClick={() => setDeleteTarget({ id: q.id, question_id: q.question_id, question: q.question })}
                                className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] px-2.5 py-1 rounded-lg bg-red-950/50 border border-red-800/50 text-red-400 hover:bg-red-900/50 hover:border-red-600 font-bold"
                              >
                                🗑 Delete
                              </button>
                            </td>
        ...
        {tab === 'stats' && (
          <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-emerald-950/40 border border-emerald-800/50 rounded-2xl p-5 mb-6">
                ...
                {activeStats.errors.length > 0 && (
                  <div className="mt-4 bg-red-950/40 rounded-xl p-3">
                    ...
                    {activeStats.errors.slice(0, 5).map((e, i) => <p key={i} className="text-xs text-red-300/70">{e}</p>)}
                  </div>
                )}
              </div>
            )}

            {slots[activeType].questions.length > 0 && (
              <>
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {[
                    { label: 'Total Questions', val: slots[activeType].questions.length, color: 'text-violet-400' },
                ...
                <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 mb-4">
                  ...
                  <div className="space-y-3">
                    {pillarStats.map(p => {
                      ...
                      const pct = Math.round((p.count / slots[activeType].questions.length) * 100);
                      return (
                        <div key={p.id}>
                          ...
                          <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-700 ${pc.dot}`} style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                      );
                    })}
                ...
                <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {uniquePersonas.map(persona => {
                      const count = slots[activeType].questions.filter(q => q.persona === persona).length;
                      return (
            ...
            {slots[activeType].questions.length === 0 && !activeStats && (
              <div className="text-center py-16 text-slate-600">
                <div className="text-4xl mb-3">📊</div>
                <p>No {activeType} data yet</p>
                <button onClick={() => setTab('import')} className="mt-4 text-xs px-4 py-2 rounded-lg bg-violet-600 text-white hover:bg-violet-500 transition">
                  → Import CSV
                </button>
              </div>
